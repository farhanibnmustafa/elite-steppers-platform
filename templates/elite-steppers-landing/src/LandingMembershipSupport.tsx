...
const memberBenefits = [
  "Exclusive member-only events & workshops",
  "Early access to Hall of Fame announcements",
  "Discounts on merch & event tickets",
] as const;

const supportPoints = [
  "Fund scholarships for young dancers",
  "Support local stepping events nationwide",
  "Preserve the history of Black stepping culture",
] as const;
      ...
      <div className={landingInnerMax}>
        <div className="flex flex-col items-center gap-3 lg:flex-row lg:items-stretch lg:justify-center lg:gap-5 xl:gap-6">
          <article className={`${cardShell} w-full max-w-sm shrink-0`}>
            <h2 className="text-[1.35rem] font-semibold leading-tight tracking-tight text-gold sm:text-[1.45rem] md:text-[1.6rem]">
              Become a Member
            </h2>
            <p className="mt-1 text-sm font-normal text-white sm:text-[0.9375rem]">
              Join the Hall of Fame Family
            </p>
            <BulletList items={memberBenefits} />
          ...
          <article className={`${cardShell} w-full max-w-sm shrink-0`}>
            ...
            <p className="mt-1 text-sm font-normal text-white sm:text-[0.9375rem]">
              Support Stepping&apos;s Future
            </p>
            <BulletList items={supportPoints} />
            <div className="mt-auto pt-4 sm:pt-4">
              <Link
                href="/donate"
                className="inline-flex items-center justify-center rounded-[10px] border border-gold bg-transparent px-4 py-1.5 text-[0.6875rem] font-medium tracking-tight text-white transition hover:bg-white/5 sm:px-5 sm:py-2 sm:text-xs"
              >
                Donate Now
              </Link>
            </div>
